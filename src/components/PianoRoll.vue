...
const zoom = computed(() => {
	...
	const minZoomX =
		(rollWidth.value - rem.value * 4) / (rem.value * props.length);
...
const longShadows = computed((): ShadowMap[] => {
	...
	return props.shadowMap.flatMap((shadow) => {
		...
		const matchingNotes = (
			Object.keys(scaleLookup.value) as OctaveNote[]
		).filter((note) => new RegExp(`${shadow.note}-?[0-9]`).test(note));
...
const length = computed(() => {
	...
	const defaultLength = Math.ceil(
		(pianoRoll.value?.getBoundingClientRect().width || 0) /
			(rem.value * zoom.value.x),
	);
	if (notes.value.length === 0) return defaultLength;
	const lastNote = notes.value.sort(
		(a, b) => b.start + b.length - (a.start + a.length),
	)[0];
	return Math.max(lastNote.start + lastNote.length + 20, defaultLength);
});

const scale = computed(() => {
	const topIndex = notesBetweenC1AndG9.findIndex(
		(note) => note === props.rangeTop,
	);
	const bottomIndex = notesBetweenC1AndG9.findIndex(
		(note) => note === props.rangeBottom,
	);
...
const notesStartingAt = (tick: number) => {
	return notes.value
		.filter((note) => beatsToTicks(note.start) === tick)
		.map((note) => note.note);
};
...
const setSelectedNotePosition = () => {
	...
	if (type === "drag") {
		note.start = Math.min(
			ticksToBeats(length.value - beatsToTicks(note.length)),
			newStart,
		);
		note.note = newNote;
		return;
	...
	if (type === "left") {
		note.start = Math.min(
			newStart,
			draggingNote.value.placement.end - ticksToBeats(1),
		);
		note.length =
			draggingNote.value.placement.end - newStart || ticksToBeats(1);
		return;
	}
...
const startAndEndNotes = (notes: OctaveNote[], timeout: number) => {
	...
	setTimeout(() => {
		...
		startedNotes.value = startedNotes.value.filter(
...
const toneGridCellWidth = computed(() => {
	return `calc(1rem * ${zoom.value.x})`;
});

const toneGridWidth = computed(() => {
	const cell = "var(--pr-tone-grid-cell-width)";
	return `calc(${cell} * ${props.length})`;
});

const toneGridBG = computed(() => {
	const width = `calc(1rem * ${zoom.value.x})`;
	const widthMinusBorder = `calc(${width} - 1px)`;
	const bg = "var(--pr-background-color)";
	const grid = "var(--pr-grid-color)";
	return `repeating-linear-gradient( to right, ${bg} 0, ${bg} ${widthMinusBorder}, ${grid} ${widthMinusBorder}, ${grid} ${width} )`;
});

const toneGridIncidentalBG = computed(() => {
	const width = `calc(1rem * ${zoom.value.x})`;
	const widthMinusBorder = `calc(${width} - 1px)`;
	const bg = "var(--pr-incidental-color)";
	const grid = "var(--pr-grid-color)";
	return `repeating-linear-gradient( to right, ${bg} 0, ${bg} ${widthMinusBorder}, ${grid} ${widthMinusBorder}, ${grid} ${width} )`;
});
...
const style = computed(() => {
	return {
		...
		"--pr-tone-grid-cell-width": toneGridCellWidth.value,
		"--pr-tone-grid-width": toneGridWidth.value,
		"--pr-tone-grid-bg": toneGridBG.value,
		"--pr-tone-grid-incidental-bg": toneGridIncidentalBG.value,
